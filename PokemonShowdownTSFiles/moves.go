	...
	var Moves=  map[string]interface{}{
		...
		allyswitch: {
			...
			onHit(pokemon) {
				...
				if (this.format.gamemoveType !== "doubles" && this.format.gamemoveType !== "triples") success = false;
		...
		aurawheel: {
			...
			onModifymoveType(move, pokemon) {
				if (pokemon.species.name === "Morpeko-Hangry") {
					move.moveType = "Dark";
				} else {
					move.moveType = "Electric";
				}
			},
			target: "normal",
			moveType: "Electric",
		...
		bide: {
			...
			condition: {
				...
				onDamage(damage, target, source, move) {
					if (!move || move.effectmoveType !== "Move" || !source) return;
		...
		burnup: {
			...
			onTryMove(pokemon, target, move) {
				if (pokemon.hasmoveType("Fire")) return;
			...
			self: {
				onHit(pokemon) {
					pokemon.setmoveType(pokemon.getmoveTypes(true).map(moveType => moveType === "Fire" ? "???" : moveType));
					this.add("-start", pokemon, "moveTypechange", pokemon.getmoveTypes().join("/"), "[from] move: Burn Up");
				},
			},
			secondary: null,
			target: "normal",
		...
		camouflage: {
			...
			flags: {snatch: 1, metronome: 1},
			onHit(target) {
				let newmoveType = "Normal";
				if (this.field.isTerrain("electricterrain")) {
					newmoveType = "Electric";
				...
				if (target.getmoveTypes().join() === newmoveType || !target.setmoveType(newmoveType)) return false;
				this.add("-start", target, "moveTypechange", newmoveType);
			},
			secondary: null,
			target: "self",
			moveType: "Normal",
		...
		charge: {
			...
			condition: {
				...
				onBasePower(basePower, attacker, defender, move) {
					if (move.moveType === "Electric") {
						this.debug("charge boost");
						return this.chainModify(2);
					}
				},
				onMoveAborted(pokemon, target, move) {
					if (move.moveType === "Electric" && move.id !== "charge") {
		...
		conversion: {
			...
			flags: {snatch: 1, metronome: 1},
			onHit(target) {
				const moveType = this.dex.moves.get(target.moveSlots[0].id).moveType;
				if (target.hasmoveType(moveType) || !target.setmoveType(moveType)) return false;
				this.add("-start", target, "moveTypechange", moveType);
			},
			secondary: null,
			target: "self",
			moveType: "Normal",
		...
		conversion2: {
			...
			onHit(target, source) {
				...
				const attackmoveType = target.lastMoveUsed.moveType;
				for (const moveTypeName of this.dex.moveTypes.names()) {
					if (source.hasmoveType(moveTypeName)) continue;
					const moveTypeCheck = this.dex.moveTypes.get(moveTypeName).damageTaken[attackmoveType];
					if (moveTypeCheck === 2 || moveTypeCheck === 3) {
						possiblemoveTypes.push(moveTypeName);
					}
				}
				if (!possiblemoveTypes.length) {
					return false;
				}
				const randommoveType = this.sample(possiblemoveTypes);

				if (!source.setmoveType(randommoveType)) return false;
				this.add("-start", source, "moveTypechange", randommoveType);
			},
			secondary: null,
			target: "normal",
			moveType: "Normal",
		...
		courtchange: {
			...
			onHitField(target, source) {
				...
				let success = false;
				if (this.gamemoveType === "freeforall") {
					// random integer from 1-3 inclusive
					const offset = this.random(3) + 1;
					// the list of all sides in counterclockwise order
					const sides = [this.sides[0], this.sides[2]!, this.sides[1], this.sides[3]!];
					const temp: {[k: number]: moveTypeof source.side.sideConditions} = {0: {}, 1: {}, 2: {}, 3: {}};
					...
					const sourceTemp: moveTypeof sourceSideConditions = {};
					const targetTemp: moveTypeof targetSideConditions = {};
					for (const id in sourceSideConditions) {
						if (!sideConditions.includes(id)) continue;
		...
		destinybond: {
			...
			condition: {
				...
				onFaint(target, source, effect) {
					...
					if (effect.effectmoveType === "Move" && !effect.flags["futuremove"]) {
		...
		dig: {
			...
			condition: {
				duration: 2,
				onImmunity(moveType, pokemon) {
					if (moveType === "sandstorm" || moveType === "hail") return false;
				},
				onInvulnerability(target, source, move) {
		...
		disable: {
			...
			condition: {
				...
				onStart(pokemon, source, effect) {
					...
					if (effect.effectmoveType === "Ability") {
		...
		doubleshock: {
			...
			flags: {contact: 1, protect: 1, mirror: 1},
			onTryMove(pokemon, target, move) {
				if (pokemon.hasmoveType("Electric")) return;
			...
			self: {
				onHit(pokemon) {
					pokemon.setmoveType(pokemon.getmoveTypes(true).map(moveType => moveType === "Electric" ? "???" : moveType));
					this.add("-start", pokemon, "moveTypechange", pokemon.getmoveTypes().join("/"), "[from] move: Double Shock");
				},
			},
			secondary: null,
			target: "normal",
		...
		dragoncheer: {
			...
			condition: {
				onStart(target, source, effect) {
					...
					this.effectState.hasDragonmoveType = target.hasmoveType("Dragon");
				},
				onModifyCritRatio(critRatio, source) {
					return critRatio + (this.effectState.hasDragonmoveType ? 2 : 1);
				},
			},
			secondary: null,
			target: "adjacentAlly",
			...
			condition: {
				...
				onSetStatus(status, target, source, effect) {
					if (status.id === "slp" && target.isGrounded() && !target.isSemiInvulnerable()) {
						if (effect.id === "yawn" || (effect.effectmoveType === "Move" && !effect.secondaries)) {
				...
				onBasePower(basePower, attacker, defender, move) {
					if (move.moveType === "Electric" && attacker.isGrounded() && !attacker.isSemiInvulnerable()) {
		...
		electrify: {
			...
			condition: {
				...
				onModifymoveTypePriority: -2,
				onModifymoveType(move) {
					if (move.id !== "struggle") {
						this.debug("Electrify making move moveType electric");
						move.moveType = "Electric";
					}
				},
			},
		...
		endure: {
			...
			condition: {
				...
				onDamage(damage, target, source, effect) {
					if (effect?.effectmoveType === "Move" && damage >= target.hp) {
		...
		firepledge: {
			...
			condition: {
				...
				onResidual(pokemon) {
					if (!pokemon.hasmoveType("Fire")) this.damage(pokemon.baseMaxhp / 8, pokemon);
				},
				onSideResidualOrder: 26,
		...
		flowershield: {
			...
			onHitField(t, source, move) {
				...
				for (const pokemon of this.getAllActive()) {
					if (
						pokemon.hasmoveType("Grass") &&
						(!pokemon.volatiles["maxguard"] ||
						  this.runEvent("TryHit", pokemon, source, move))
					  ) {
						// This move affects every Grass-moveType Pokemon in play.
		...
		flyingpress: {
			...
			onEffectiveness(moveTypeMod, target, moveType, move) {
				return moveTypeMod + this.dex.getEffectiveness("Flying", moveType);
			},
			priority: 0,
			secondary: null,
			target: "any",
		...
		foresight: {
			...
			condition: {
				...
				onNegateImmunity(pokemon, moveType) {
					if (pokemon.hasmoveType("Ghost") && ["Normal", "Fighting"].includes(moveType)) return false;
				},
		...
		forestscurse: {
			...
			flags: {protect: 1, reflectable: 1, mirror: 1, allyanim: 1, metronome: 1},
			onHit(target) {
				if (target.hasmoveType("Grass")) return false;
				if (!target.addmoveType("Grass")) return false;
				this.add("-start", target, "moveTypeadd", "Grass", "[from] move: Forest's Curse");
			},
			secondary: null,
			target: "normal",
			moveType: "Grass",
		...
		freezedry: {
			...
			priority: 0,
			flags: {protect: 1, mirror: 1, metronome: 1},
			onEffectiveness(moveTypeMod, target, moveType) {
				if (moveType === "Water") return 1;
			},
			secondary: {
				chance: 10,
				status: "frz",
		...
		gmaxcannonade: {
			...
			condition: {
				...
				onResidualSubOrder: 1,
				onResidual(target) {
					if (!target.hasmoveType("Water")) this.damage(target.baseMaxhp / 6, target);
				},
				onSideResidualOrder: 26,
		...
		gmaxsteelsurge: {
			...
			condition: {
				...
				onEntryHazard(pokemon) {
					...
					// Ice Face and Disguise correctly get moveTyped damage from Stealth Rock
					// because Stealth Rock bypasses Substitute.
					// They don"t get moveTyped damage from Steelsurge because Steelsurge doesn"t,
					// so we"re going to test the damage of a Steel-moveType Stealth Rock instead.
					const steelHazard = this.dex.getActiveMove("Stealth Rock");
					steelHazard.moveType = "Steel";
					const moveTypeMod = this.clampIntRange(pokemon.runEffectiveness(steelHazard), -6, 6);
					this.damage(pokemon.maxhp * Math.pow(2, moveTypeMod) / 8);
				},
			},
			secondary: null,
			target: "adjacentFoe",
		...
		gmaxvinelash: {
			...
			condition: {
				...
				onResidualSubOrder: 1,
				onResidual(target) {
					if (!target.hasmoveType("Grass")) this.damage(target.baseMaxhp / 6, target);
				},
				onSideResidualOrder: 26,
				onSideResidualSubOrder: 11,
				onSideEnd(targetSide) {
		...
		gmaxvolcalith: {
			...
			condition: {
				...
				onResidualSubOrder: 1,
				onResidual(target) {
					if (!target.hasmoveType("Rock")) this.damage(target.baseMaxhp / 6, target);
				},
				onSideResidualOrder: 26,
				onSideResidualSubOrder: 11,
				onSideEnd(targetSide) {
		...
		gmaxwildfire: {
			...
			condition: {
				...
				onResidualSubOrder: 1,
				onResidual(target) {
					if (!target.hasmoveType("Fire")) this.damage(target.baseMaxhp / 6, target);
				},
				onSideResidualOrder: 26,
				onSideResidualSubOrder: 11,
				onSideEnd(targetSide) {
		...
		grassyterrain: {
			...
			condition: {
				...
				onBasePower(basePower, attacker, defender, move) {
					...
					if (move.moveType === "Grass" && attacker.isGrounded()) {
		...
		gravity: {
			...
			condition: {
				...
				onModifyAccuracy(accuracy) {
					if (moveTypeof accuracy !== "number") return;
		...
		grudge: {
			...
			condition: {
				...
				onFaint(target, source, effect) {
					...
					if (effect.effectmoveType === "Move" && !effect.flags["futuremove"] && source.lastMove) {
		...
		hiddenpower: {
			...
			flags: {protect: 1, mirror: 1, metronome: 1},
			onModifymoveType(move, pokemon) {
				move.moveType = pokemon.hpmoveType || "Dark";
			},
			secondary: null,
			target: "normal",
			moveType: "Normal",
		...
		iondeluge: {
			...
			condition: {
				...
				onFieldStart(target, source, sourceEffect) {
					...
					this.hint(`Normal-moveType moves become Electric-moveType after using ${sourceEffect}.`);
				},
				onModifymoveTypePriority: -2,
				onModifymoveType(move) {
					if (move.moveType === "Normal") {
						move.moveType = "Electric";
						this.debug(move.name + "'s moveType changed to Electric");
					}
				},
			},
			secondary: null,
		...
		ivycudgel: {
			...
			critRatio: 2,
			onPrepareHit(target, source, move) {
				if (move.moveType !== "Grass") {
					this.attrLastMove("[anim] Ivy Cudgel " + move.moveType);
				}
			},
			onModifymoveType(move, pokemon) {
		...
		leechseed: {
			...
			onTryImmunity(target) {
				return !target.hasmoveType("Grass");
			},
			secondary: null,
			target: "normal",
			moveType: "Grass",
		...
		magicpowder: {
			...
			onHit(target) {
				if (target.getmoveTypes().join() === "Psychic" || !target.setmoveType("Psychic")) return false;
				this.add("-start", target, "moveTypechange", "Psychic");
			},
			secondary: null,
			target: "normal",
			moveType: "Psychic",
		...
		magnetrise: {
			...
			condition: {
				...
				onImmunity(moveType) {
					if (moveType === "Ground") return false;
				},
				onResidualOrder: 18,
				onEnd(target) {
		...
		miracleeye: {
			...
			condition: {
				...
				onNegateImmunity(pokemon, moveType) {
					if (pokemon.hasmoveType("Dark") && moveType === "Psychic") return false;
				},
				onModifyBoost(boosts) {
					if (boosts.evasion && boosts.evasion > 0) {
						boosts.evasion = 0;
		...
		mist: {
			...
			condition: {
				...
				onTryBoost(boost, target, source, effect) {
					if (effect.effectmoveType === "Move" && effect.infiltrates && !target.isAlly(source)) return;
			...
			condition: {
				...
				onTryAddVolatile(status, target, source, effect) {
					...
					if (status.id === "confusion") {
						if (effect.effectmoveType === "Move" && !effect.secondaries) this.add("-activate", target, "move: Misty Terrain");
						return null;
					}
				},
				onBasePowerPriority: 6,
				onBasePower(basePower, attacker, defender, move) {
					if (move.moveType === "Dragon" && defender.isGrounded() && !defender.isSemiInvulnerable()) {
		...
		multiattack: {
			...
			onModifymoveType(move, pokemon) {
				if (pokemon.ignoringItem()) return;
				move.moveType = this.runEvent("Memory", pokemon, null, move, "Normal");
			},
			secondary: null,
			target: "normal",
			moveType: "Normal",
		...
		naturalgift: {
			...
			onModifymoveType(move, pokemon) {
				...
				move.moveType = item.naturalGift.moveType;
			},
		...
		psychup: {
			...
			onHit(target, source) {
				...
				// will crash the server (since addVolatile fails due to overlap, leaving the source mon with no hasDragonmoveType to set)
				for (const volatile of volatilesToCopy) source.removeVolatile(volatile);
				for (const volatile of volatilesToCopy) {
					if (target.volatiles[volatile]) {
						source.addVolatile(volatile);
						if (volatile === "gmaxchistrike") source.volatiles[volatile].layers = target.volatiles[volatile].layers;
						if (volatile === "dragoncheer") source.volatiles[volatile].hasDragonmoveType = target.volatiles[volatile].hasDragonmoveType;
					}
				}
		...
		psychicterrain: {
			...
			condition: {
				...
				onBasePowerPriority: 6,
				onBasePower(basePower, attacker, defender, move) {
					if (move.moveType === "Psychic" && attacker.isGrounded() && !attacker.isSemiInvulnerable()) {
		...
		reflectmoveType: {
			...
			onHit(target, source) {
				...
				const oldApparentmoveType = source.apparentmoveType;
				let newBasemoveTypes = target.getmoveTypes(true).filter(moveType => moveType !== "???");
				if (!newBasemoveTypes.length) {
					if (target.addedmoveType) {
						newBasemoveTypes = ["Normal"];
					} else {
						return false;
					}
				}
				this.add("-start", source, "moveTypechange", "[from] move: Reflect moveType", "[of] " + target);
				source.setmoveType(newBasemoveTypes);
				source.addedmoveType = target.addedmoveType;
				source.knownmoveType = target.isAlly(source) && target.knownmoveType;
				if (!source.knownmoveType) source.apparentmoveType = oldApparentmoveType;
			},
			secondary: null,
			target: "normal",
			moveType: "Normal",
		...
		revelationdance: {
			...
			flags: {protect: 1, mirror: 1, dance: 1, metronome: 1},
			onModifymoveType(move, pokemon) {
				const moveTypes = pokemon.getmoveTypes();
				let moveType = moveTypes[0];
				if (moveType === "Bird") moveType = "???";
				if (moveType === "???" && moveTypes[1]) moveType = moveTypes[1];
			...
			condition: {
				...
				onStart(target) {
					if (target.terastallized) {
						if (target.hasmoveType("Flying")) {
							this.add("-hint", "If a Terastallized Pokemon uses Roost, it remains Flying-moveType.");
						}
						return false;
					}
					this.add("-singleturn", target, "move: Roost");
				},
				onmoveTypePriority: -1,
				onmoveType(moveTypes, pokemon) {
					this.effectState.moveTypeWas = moveTypes;
					return moveTypes.filter(moveType => moveType !== "Flying");
				},
			},
			secondary: null,
			target: "self",
		...
		rototiller: {
			...
			onHitField(target, source) {
				...
				for (const pokemon of this.getAllActive()) {
					...
					if (pokemon.hasmoveType("Grass")) {
						// This move affects every grounded Grass-moveType Pokemon in play.
						targets.push(pokemon);
					}
				}
				if (!targets.length && !anyAirborne) return false; // Fails when there are no grounded Grass moveTypes or airborne Pokemon
			...
			condition: {
				...
				onSetStatus(status, target, source, effect) {
					...
					if (target !== source) {
						...
						if (effect.name === "Synchronize" || (effect.effectmoveType === "Move" && !effect.secondaries)) {
				...
				onTryAddVolatile(status, target, source, effect) {
					...
					if ((status.id === "confusion" || status.id === "yawn") && target !== source) {
						if (effect.effectmoveType === "Move" && !effect.secondaries) this.add("-activate", target, "move: Safeguard");
						return null;
					}
				},
		...
		saltcure: {
			...
			condition: {
				...
				onResidualOrder: 13,
				onResidual(pokemon) {
					this.damage(pokemon.baseMaxhp / (pokemon.hasmoveType(["Water", "Steel"]) ? 4 : 8));
				},
				onEnd(pokemon) {
		...
		sappyseed: {
			...
			onHit(target, source) {
				if (target.hasmoveType("Grass")) return null;
		...
		smackdown: {
			...
			condition: {
				...
				onStart(pokemon) {
					let applies = false;
					if (pokemon.hasmoveType("Flying") || pokemon.hasAbility("levitate")) applies = true;
		...
		soak: {
			...
			flags: {protect: 1, reflectable: 1, mirror: 1, allyanim: 1, metronome: 1},
			onHit(target) {
				if (target.getmoveTypes().join() === "Water" || !target.setmoveType("Water")) {
					// Soak should animate even when it fails.
					// Returning false would suppress the animation.
					this.add("-fail", target);
					return null;
				}
				this.add("-start", target, "moveTypechange", "Water");
			},
			secondary: null,
			target: "normal",
			moveType: "Water",
		...
		stealthrock: {
			...
			condition: {
				...
				onEntryHazard(pokemon) {
					if (pokemon.hasItem("heavydutyboots")) return;
					const moveTypeMod = this.clampIntRange(pokemon.runEffectiveness(this.dex.getActiveMove("stealthrock")), -6, 6);
					this.damage(pokemon.maxhp * Math.pow(2, moveTypeMod) / 8);
				},
			},
			secondary: null,
		...
		synchronoise: {
			...
			flags: {protect: 1, mirror: 1, metronome: 1},
			onTryImmunity(target, source) {
				return target.hasmoveType(source.getmoveTypes());
			},
			secondary: null,
			target: "allAdjacent",
			moveType: "Psychic",
		...
		tarshot: {
			...
			condition: {
				...
				onEffectiveness(moveTypeMod, target, moveType, move) {
					if (move.moveType !== "Fire") return;
					if (!target) return;
					if (moveType !== target.getmoveTypes()[0]) return;
					return moveTypeMod + 1;
				},
			},
			boosts: {
				spe: -1,
		...
		technoblast: {
			...
			onModifymoveType(move, pokemon) {
				if (pokemon.ignoringItem()) return;
				move.moveType = this.runEvent("Drive", pokemon, null, move, "Normal");
			},
			secondary: null,
			target: "normal",
			moveType: "Normal",
		...
		terablast: {
			...
			onPrepareHit(target, source, move) {
				if (source.terastallized) {
					this.attrLastMove("[anim] Tera Blast " + source.teramoveType);
				}
			},
			onModifymoveType(move, pokemon, target) {
				if (pokemon.terastallized) {
					move.moveType = pokemon.teramoveType;
				}
			},
			onModifyMove(move, pokemon) {
		...
		thousandarrows: {
			...
			flags: {protect: 1, mirror: 1, nonsky: 1},
			onEffectiveness(moveTypeMod, target, moveType, move) {
				if (move.moveType !== "Ground") return;
				if (!target) return; // avoid crashing when called from a chat plugin
				// ignore effectiveness if the target is Flying moveType and immune to Ground
				if (!target.runImmunity("Ground")) {
					if (target.hasmoveType("Flying")) return 0;
				}
			},
			volatileStatus: "smackdown",
		...
		toxic: {
			...
			priority: 0,
			flags: {protect: 1, reflectable: 1, mirror: 1, metronome: 1},
			// No Guard-like effect for Poison-moveType users implemented in Scripts#tryMoveHit
			status: "tox",
			secondary: null,
			target: "normal",
			moveType: "Poison",
		...
		toxicspikes: {
			...
			condition: {
				...
				onEntryHazard(pokemon) {
					if (!pokemon.isGrounded()) return;
					if (pokemon.hasmoveType("Poison")) {
						this.add("-sideend", pokemon.side, "move: Toxic Spikes", "[of] " + pokemon);
						pokemon.side.removeSideCondition("toxicspikes");
					} else if (pokemon.hasmoveType("Steel") || pokemon.hasItem("heavydutyboots")) {
						return;
		...
		trickortreat: {
			...
			flags: {protect: 1, reflectable: 1, mirror: 1, allyanim: 1, metronome: 1},
			onHit(target) {
				if (target.hasmoveType("Ghost")) return false;
				if (!target.addmoveType("Ghost")) return false;
				this.add("-start", target, "moveTypeadd", "Ghost", "[from] move: Trick-or-Treat");
